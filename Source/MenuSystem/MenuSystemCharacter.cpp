...
void AMenuSystemCharacter::OnCreateSessionComplete(FName SessionName, bool BWasSuccessful)
{
	if(BWasSuccessful)
	{
		...
		if (World)
		{
			World->ServerTravel(FString("/Game/ThirdPerson/Maps/Lobby?listen")); //open the lobby level as a listen server
		}
...
void AMenuSystemCharacter::OnFindSessionsComplete(bool BWasSuccessful)
{
	...
	// search results is a tarray so we can loop through it
		...
		if (MatchType == FString("FreeForAll")) 
		{
			if (GEngine)
			{
				GEngine->AddOnScreenDebugMessage(
					...
					15.f,
					FColor::Cyan,
					FString::Printf(TEXT("Id: %s, User : %s"), *MatchType) //checking the matchtype
				);
			}
